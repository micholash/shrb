/* =============================================
   ...
   - AI 난이도 3단계 (깊이 1/3/5)
   - 완전한 장기 규칙 구현
   - Firebase Realtime Database 게임 결과 저장
   ============================================= */
...
// 게임 결과를 Firebase RT DB에 저장
// result: 'win' | 'lose' | 'draw'
// reason: '외통수' | '빅장' | '한수쉼'
async function saveGameResult(result, reason) {
  try {
    const record = {
      result,                          // 플레이어 기준 승/패/무
      reason,                          // 종료 사유
      playerSide,                      // 플레이어 진영 ('G'=초 / 'R'=한)
      aiDepth,                         // AI 난이도
      scoreG: calcScore('G'),          // 초 점수
      scoreR: calcScore('R'),          // 한 점수
      capturedByG: capturedG.map(p => p.name),
      capturedByR: capturedR.map(p => p.name),
      timestamp: Date.now(),
      date: new Date().toISOString(),
    };
    await push(ref(db, 'janggi/results'), record);
    console.log('[Firebase] 게임 결과 저장 완료:', record);
  } catch (err) {
...
function initBoard() {
  ...
  board[1][4]=mk('G','king','楚'); board[1][3]=mk('G','advisor','士'); board[0][5]=mk('G','advisor','士');
  ...
  board[8][4]=mk('R','king','漢'); board[8][3]=mk('R','advisor','士'); board[9][5]=mk('R','advisor','士');
...
function passTurn() {
  ...
  passCount++;
  if (passCount >= 2) { showModal('무승부','두 플레이어 모두 한수쉬어 무승부입니다.'); saveGameResult('draw', '한수쉼'); return; }
  currentTurn   = currentTurn==='G' ? 'R' : 'G';
  selected      = null;
  possibleMoves = [];
  updateTurnUI();
...
function checkGameState() {
  if (isCheckmate(currentTurn)) {
    ...
    gameStarted = false;
    saveGameResult(isPlayerWin ? 'win' : 'lose', '외통수');
    return;
  }
  if (isBikjang()) {
...
// type="module"은 전역 scope가 닫혀 있어서
// HTML onclick="..."에서 함수를 못 찾음 → window에 수동 노출
